...
Matrix createLocalK(int e,mesh &m){
    ...
    //Preparando matrixL (No existia) --> modificada
    ...
    //Preparando matrixG (En clase conocida simplemente como G) --> modificada
...
Vector createLocalb(int e,mesh &m){
    ...
    float eta = m.getParameter(ETA); //Nuevo
...
void assemblyK(element e,Matrix localK,Matrix &K,int nnodes){
	...
	index5 = index2+nnodes, 
	index6 = index3+nnodes;
    int index7 = index1+2*nnodes, 
	index8 = index2+2*nnodes, 
